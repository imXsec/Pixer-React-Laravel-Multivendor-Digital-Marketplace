...
class ShopController extends CoreController
{
    ...
    public function topShops(Request $request)
    {
        ...
        // $shops_query = $this->repository->leftJoin('orders', 'shops.id', '=', 'orders.shop_id');
        // if ($range) {
        //     $shops_query = $shops_query->where('orders.created_at', '>', Carbon::now()->subDays($range));
        // }
        // return $shops_query->withCount(['orders'])->orderBy('orders_count', 'desc')->groupBy('shops.id')->paginate($limit);
    }
}
